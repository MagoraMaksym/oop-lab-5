...
bool Vehicle::setName(std::string name) { 
    m_name_of_vehicle = name; return true; 
}
...
Truck::Truck(int cap) : Vehicle() {
     std::cout << "Constructor: Truck \n"; 
    }
Truck::~Truck() { 
...
Pickup::~Pickup() { 
    std::cout << "Destructor: Pickup\n";
 }
void Pickup::StartEngine() { 
    std::cout << "\nПікап заведено!";
 }

ElectricCar::ElectricCar() : Car(0) { 
    setName("EV"); 
}
